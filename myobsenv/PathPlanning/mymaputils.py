...
#obstacle map
class myMap:
    def __init__(self,obstacle_list,SensorNodes,path_resolution = 0.5,dim = 3):
        self.dim = dim                              # 3D
        self.idx = self.get_tree(obstacle_list,dim) # bounds 为障碍物下限上限
        self.len = len(obstacle_list)               # 障碍物数目
        self.path_res = path_resolution             # 路径分辨率
        self.obstacles = obstacle_list              # 障碍物list
        # self.bounds = bounds                        # 自己设定的地图上下限制
        self.SensorNodes = SensorNodes              # 自己设定的SensorNodes
    ...
    def plotSensors(self,ax):                   # 标记sensors
        SensorNodes = np.array(self.SensorNodes)      # scale 图是scale后的
        if self.dim == 2:
            print("DIM = 2 !!")
        elif self.dim == 3:
            node_i = 1
            for sensor in SensorNodes:
                ax.scatter(sensor[0], sensor[1], sensor[-1], c='r')
                ax.text(sensor[0]+0.2, sensor[1]+0.2, sensor[-1]+0.2, str(node_i), color='red') # 标记传感器节点
                # ax.annotate(node_i, xy = (sensor[0], sensor[1], sensor[-1]), xytext = (sensor[0]+0.1, sensor[1]+0.1, sensor[-1]+0.1))
                node_i = node_i + 1   # 传感器节点No.
                # ax.lines[-1].set_3d_properties(sensor[-1])
                # X, Y, Z = cuboid_data(box)
                # ax.plot_surface(X, Y, Z, rstride=1, cstride=1,color=(0.1, 0.15, 0.3, 0.2),zorder = 1)
        else: print('can not plot for given dimensions')
