...
def myenvobsv1Env_register():
    register(
        id='myenvobs-v1',
        entry_point='myobsenv.myenvobs_v1:myenvobsEnv',
...
class myenvobsEnv(gym.Env):
    ...
    def _obs_def(self, min_pos, max_pos, dim, MaxNumObs, SensorNum, SensorMaxContent):
        ...
        observation_space = spaces.Box(low=0, high=self.size, shape=(4, self.max_pos[0] - self.min_pos[0], self.max_pos[1] - self.min_pos[1]), dtype=np.float)
    ...
    def _get_obs_2d(self):
        ...
        StateSensorMap = np.zeros((self.max_pos[0] - self.min_pos[0], self.max_pos[1] - self.min_pos[1]))
        # 2D 图，3D Sensor图压扁，元素值为高度
        for i in self.SensorObjList: # x,y,z
            StateSensorMap[i.Location[0], i.Location[1]] = i.Location[2]
        
        StateObsMap = np.zeros((self.max_pos[0] - self.min_pos[0], self.max_pos[1] - self.min_pos[1]))
        # 2D 图，3D障碍物图压扁，元素值为高度
        for Obs_i in self.MyRandomenv.obstacles: # 3D boxes   lx, ly, lz, hx, hy, hz
            for x in range(Obs_i[0], Obs_i[3]):
                for y in range(Obs_i[1], Obs_i[4]):
                    StateObsMap[x, y] = Obs_i[5]

        StateSensorContentMap = np.zeros((self.max_pos[0] - self.min_pos[0], self.max_pos[1] - self.min_pos[1]))
        # 2D 图，3D Sensor图压扁，元素值为Sensor信息量
        for i in self.SensorObjList: # x,y,content
            StateSensorContentMap[i.Location[0], i.Location[1]] = i.Content

        UAVLocMap = np.zeros((self.max_pos[0] - self.min_pos[0], self.max_pos[1] - self.min_pos[1]))         # 2D , UAV 坐标
        UAVLocTemp = np.clip(np.array([int(self._agent_location[0]), int(self._agent_location[1])]), self.min_pos[:2], self.max_pos[:2]-1)
        UAVLocMap[UAVLocTemp[0], UAVLocTemp[1]] = self._agent_location[2]
        ...
        StateMap = np.concatenate((StateObsMap[np.newaxis, :, :], 
                                            StateSensorMap[np.newaxis, :, :], 
                                            StateSensorContentMap[np.newaxis, :, :], 
                                            UAVLocMap[np.newaxis, :, :]), axis=0)
        return StateMap
    ...
    def _reward_cal(self, old_loc, new_loc):
        ...
        if np.linalg.norm(old_loc - new_loc) < 0.1:
            reward += -0.01
        
        for i in self.SensorObjList:
            dist = np.linalg.norm(self._agent_location - i.Location)
            if i.TotalContent != 0 and dist < 1:
                reward += 6
                i.TotalContent = 0
    ...
    def step(self, action):
        ...
        if (np.linalg.norm(self._agent_location - self._target_location) < 1 and self.NumOfStep > 1) or self.NumOfStep > 3000:
            # reward = 10
            terminated = True
